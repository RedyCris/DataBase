...
// NOLINTNEXTLINE
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args) -> AbstractExpressionRef {
    // 1. check if the parsed function name is "lower" or "upper"
    // 2. verify the number of args (should be 1), refer to the test cases for when you shoul
    //3.return a`stringExpression`std::shared ptr.
    
    if (args.size() != 1 || args[0]->GetReturnType().GetType() != TypeId::VARCHAR) {
    ...
    {StringExpressionType expr_type = (func_name == "lower") ? StringExpressionType::Lower : StringExpressionType::Upper;
        ...
        throw Exception(fmt::format("Function {} not supported in planner yet", func_name));
    }
    }    
